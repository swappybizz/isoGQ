...
st.set_page_config(
    page_title="Company Introduction",
    page_icon="👋",
    layout="wide"
)
st.warning("ISOGRAPH stiller omtrent 20 spørsmål for å samle foreløpige data til ISOEnsure. Assistenten kan gjøre mindre justeringer i svaret basert på dine innspill. Du kan også laste opp et dokument for å hjelpe med å besvare spørsmålene.")
# st.title("ISOGRAPH")
...
if len(st.session_state.messages) == 0:
    st.session_state.messages.append({"role": "assistant", "content": "Hei, jeg skal hjelpe deg med å komme i gang med ISOEnsure. Hva er navnet på organisasjonen din? Kan du oppgi organisasjonsnummeret og et kontaktpunkt (e-post/person)?"})
    ...
    with st.status(".", expanded=True):
        ...
        if len(st.session_state.uploaded_files) > 0:
            for doc in st.session_state.uploaded_files:
                ...
                doc_content += "\n"
                st.session_state.uploaded_files_content.append(doc_content)
        print(doc_content)
...
with st.sidebar:
    st.title("Introduction of Company")
    st.subheader("Helps gather preliminary Data for ISOEnsure")
...
"""
    ...
    st.download_button(
            label="Download Questionaire as a Text File",
            data=questions,
